...
# Simple Step-by-Step Demo Script
...
run_step() {
    ...
    if [ $attempts -eq 30 ]; then
        if [ "$step" = "2" ]; then
            print_step "Service was killed by OOM (Out of Memory) - This is EXPECTED for Step 2!"
...
# Main demo
main() {
    print_header "Simple Chaos Engineering Demo"
    echo "=================================="
    echo ""
    ...
    echo "• Create session: curl http://localhost:8083/session/create"
    ...
    echo "🎓 EDUCATIONAL CONTENT - Step 4:"
    echo "I've built this service to demonstrate database connectivity challenges."
    echo "It manages user data and tries to connect to MySQL database,"
    echo "but MySQL isn't running. The service will fail but shows proper error handling."
    echo ""
    echo "🎯 What I'm going to show you:"
    ...
    echo "• Create user: curl -X POST http://localhost:8084/user/create"
    echo "• User endpoint: http://localhost:8084/user/create"
    ...
    echo "comprehensive monitoring, proper error handling, fallback mechanisms,"
    echo "and all the best practices we've learned."
    echo ""
    echo "What to show the audience:"
    echo "• Health endpoint: curl http://localhost:8085/health"
    echo "• Debug info: curl http://localhost:8085/debug"
    echo "• Run experiment: curl http://localhost:8085/run-experiment"
    ...
    echo "• Step 1: http://localhost:8081 (Network Failure)"
    echo "• Step 2: http://localhost:8082 (Resource Failure)"
    echo "• Step 3: http://localhost:8083 (Service Failure)"
    echo "• Step 4: http://localhost:8084 (Database Failure)"
    echo "• Step 5: http://localhost:8085 (Success)"
    echo ""
    echo "You can continue to interact with these services and explain"
    echo "the educational content to your audience."
    echo ""
    echo "To clean up all containers when done:"
    echo "  ./cleanup.sh"
}
