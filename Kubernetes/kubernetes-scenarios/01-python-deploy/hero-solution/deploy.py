...
""")
    ...
    if env == "minikube":
        ...
        print(f"   🎯 Try NodePort: http://localhost:{node_port}")
        print(f"   ℹ️  If that doesn't work, use port-forward below:")
    else:
        print(f"   🎯 Try NodePort: http://localhost:{node_port}")
        print(f"   ℹ️  If that doesn't work, use port-forward below:")

    # Always show port-forward commands (universal access method)
    print("\n🔧 PORT-FORWARD COMMANDS (Copy & Paste - Works on All Platforms):\n")
    print(f"   # Terminal 1: Start port-forward for voting app")
    print(f"   kubectl port-forward -n voting-app svc/vote-app 31004:80")
    print()
    print(f"   # Then open in browser: http://localhost:31004")
    print()
    print(f"   # Keep the terminal open while using the app!")
    ...
    print("\n🎮 HOW TO USE:\n")
    print("   1. Run the port-forward command above (in a new terminal)")
    print("   2. Open http://localhost:31004 in your browser")
    print("   3. Click 🏠 WFH or 🏢 WFO to vote")
    print("   4. Watch the results update in real-time!")
    print("   5. Open multiple tabs and vote - all votes are counted")

    print("\n🧪 TEST FROM COMMAND LINE:\n")
    print("   # Vote for WFH")
    print("   curl -X POST http://localhost:31004/vote -H 'Content-Type: application/json' -d '{\"option\":\"WFH\"}'")
    print()
    print("   # Vote for WFO")
    print("   curl -X POST http://localhost:31004/vote -H 'Content-Type: application/json' -d '{\"option\":\"WFO\"}'")
    print()
    print("   # Check results")
    print("   curl http://localhost:31004/results")

    print("\n🧹 CLEANUP WHEN DONE:\n")
    print("   kubectl delete namespace voting-app")
    print("   # Stop port-forward with Ctrl+C in its terminal")
